...
var AuthService = class _AuthService {
  ...
  async login(email, password) {
    try {
      console.log("AuthService: Querying database for email:", email);
      const result = await firstValueFrom(this.db.query(GET_ADMIN_USER_BY_EMAIL, [email]));
      console.log("AuthService: Raw Database query result:", result);
      console.log("AuthService: Type of result:", typeof result);
      console.log("AuthService: result.data:", result?.data);
      console.log("AuthService: Type of result.data:", typeof result?.data);
      console.log("AuthService: result.data.length:", result?.data?.length);
      if (result?.data && result?.data?.length > 0) {
        const user = result.data[0];
        console.log("AuthService: User found:", user);
        if (user.password === password) {
          console.log("AuthService: Password match. Login successful.");
...
var AdminLoginComponent = class _AdminLoginComponent {
  ...
  async login() {
    ...
    try {
      ...
      console.log("AuthService.login returned success:", success);
      if (success) {
        console.log("Login successful, navigating to dashboard.");
        this.router.navigate(["/admin/dashboard"]);
      } else {
        this.errorMessage = "Invalid email or password.";
        console.log("Login failed, error message set:", this.errorMessage);
      }
    } catch (error) {
      this.errorMessage = "An unexpected error occurred. Please try again.";
      console.error("Login error in AdminLoginComponent:", error);
    }
  }
...
//# sourceMappingURL=chunk-CHO5GUBS.js.map
